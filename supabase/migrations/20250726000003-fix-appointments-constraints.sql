-- Fix appointments table constraints and ensure create_booking_admin works correctly
-- This migration addresses the "appointments_service_status_check" constraint violation

-- 1. First, let's check what values are currently being inserted that violate the constraint
-- The issue is likely that we're inserting a value that doesn't match the constraint exactly

-- 2. Update the create_booking_admin function to use the correct constraint values
...
BEGIN
    ...
    -- Get client record and validate
    SELECT * INTO client_record FROM clients WHERE user_id = _client_user_id;
    IF client_record IS NULL THEN
        RAISE EXCEPTION 'Client record not found for user_id: %', _client_user_id;
    END IF;
    
    client_id := client_record.id;
    RAISE NOTICE '[create_booking_admin] Found client: id=%, user_id=%, name=%', client_id, client_record.user_id, client_record.name;
    ...
    -- Use explicit values that match the constraints exactly
    INSERT INTO appointments (
        ...
        _time_slot, 
        _notes,
        'pending',  -- Must be one of: 'pending', 'confirmed', 'cancelled', 'in_progress', 'completed'
        'not_started',  -- Must be one of: 'not_started', 'in_progress', 'completed'
        service_duration,
        final_price,
        _override_conflicts,
        TRUE
    ...
    RAISE NOTICE '[create_booking_admin] APPOINTMENT CREATED: id=%, client_id=%, duration=%, price=%, is_admin_override=%', 
        new_appointment_id, client_id, service_duration, final_price, _override_conflicts;
    ...
    RAISE NOTICE '[create_booking_admin] SUCCESS: appointment_id=%, client_id=%, override_slots_tracked=%', 
        new_appointment_id, client_id, override_slots_created;
...
-- 3. Add helpful comments for future maintainers
COMMENT ON FUNCTION public.create_booking_admin IS
'Admin booking function with override support. Handles multi-role staff assignments and tracks override slots. Uses appointment_staff table for staff assignments.'; 