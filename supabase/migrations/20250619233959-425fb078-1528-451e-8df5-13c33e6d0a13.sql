...
-- Fix the remaining ambiguous provider_id reference in handle_new_user function
...
BEGIN
    ...
    ELSIF user_role = 'groomer' THEN
        ...
        -- Always get the provider_id (whether new or existing)
        SELECT id INTO provider_id
        FROM public.provider_profiles
        WHERE user_id = NEW.id AND type = 'groomer';
        ...
        IF provider_id IS NOT NULL THEN
            IF NOT EXISTS (
                ...
                WHERE provider_availability.provider_id = provider_id
                LIMIT 1
            ) THEN
    ...
    ELSIF user_role = 'vet' THEN
        ...
        SELECT id INTO provider_id
        FROM public.provider_profiles
        WHERE user_id = NEW.id AND type = 'vet';
