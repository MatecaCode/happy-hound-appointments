-- Clean up and fix the booking system to use only new tables
-- Remove all legacy booking functions and recreate with new schema

-- First, drop the current create_booking_atomic that still uses legacy tables
DROP FUNCTION IF EXISTS public.create_booking_atomic(uuid, uuid, uuid, uuid[], date, time without time zone, text);

-- Create the new create_booking_atomic function using only the new schema
CREATE OR REPLACE FUNCTION public.create_booking_atomic(
    _user_id uuid,
    _pet_id uuid,
    _service_id uuid,
    _provider_ids uuid[], -- These are actually staff_profile_ids
    _booking_date date,
...
BEGIN
    ...
    IF array_length(_provider_ids, 1) > 0 THEN
        FOREACH staff_profile_id IN ARRAY _provider_ids LOOP
            RAISE NOTICE '[create_booking_atomic] VALIDATING STAFF AVAILABILITY: staff_id=%', staff_profile_id;
            ...
            WHILE check_minutes < service_duration LOOP
                ...
                IF NOT EXISTS(
                    ...
                    RAISE EXCEPTION 'Staff % not available for date % at time %', staff_profile_id, _booking_date, check_time;
                END IF;
                
                check_minutes := check_minutes + 30;
            END LOOP;
            
            RAISE NOTICE '[create_booking_atomic] STAFF AVAILABILITY: CONFIRMED for staff_id=%', staff_profile_id;
        END LOOP;
    END IF;
    ...
    IF array_length(_provider_ids, 1) > 0 THEN
        FOREACH staff_profile_id IN ARRAY _provider_ids LOOP
            ...
            VALUES (new_appointment_id, staff_profile_id, 'primary');
            
            RAISE NOTICE '[create_booking_atomic] STAFF LINKED: appointment_id=%, staff_id=%', new_appointment_id, staff_profile_id;
        END LOOP;
    END IF;
    ...
    IF array_length(_provider_ids, 1) > 0 THEN
        FOREACH staff_profile_id IN ARRAY _provider_ids LOOP
            ...
            RAISE NOTICE '[create_booking_atomic] STAFF AVAILABILITY UPDATED: staff_id=%, date=%, time=%', staff_profile_id, _booking_date, _time_slot;
        END LOOP;
    END IF;
...
-- Also remove any other legacy functions that use old tables
DROP FUNCTION IF EXISTS public.ensure_provider_availability(uuid, date, date);
DROP FUNCTION IF EXISTS public.ensure_shower_availability(date, date);
DROP FUNCTION IF EXISTS public.get_available_providers(uuid, date, time without time zone, integer);
DROP FUNCTION IF EXISTS public.get_available_slots_for_service(uuid, date, uuid);