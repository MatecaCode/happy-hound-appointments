...
-- Enhanced debug logging for create_booking_atomic with forced error details
...
BEGIN
  ...
  -- VALIDATE BASIC INPUTS EXIST
  SELECT EXISTS(SELECT 1 FROM auth.users WHERE id = _user_id) INTO user_exists;
  IF NOT user_exists THEN
    RAISE EXCEPTION 'USER_NOT_FOUND: User % does not exist in auth.users', _user_id;
  END IF;
  RAISE NOTICE '[CREATE_BOOKING_ATOMIC] User validation: PASSED';

  SELECT EXISTS(SELECT 1 FROM pets WHERE id = _pet_id AND user_id = _user_id) INTO pet_exists;
  IF NOT pet_exists THEN
    RAISE EXCEPTION 'PET_NOT_FOUND: Pet % does not exist or does not belong to user %', _pet_id, _user_id;
  END IF;
  RAISE NOTICE '[CREATE_BOOKING_ATOMIC] Pet validation: PASSED';

  SELECT EXISTS(SELECT 1 FROM services WHERE id = _service_id) INTO service_exists;
  IF NOT service_exists THEN
    RAISE EXCEPTION 'SERVICE_NOT_FOUND: Service % does not exist', _service_id;
  END IF;
  RAISE NOTICE '[CREATE_BOOKING_ATOMIC] Service validation: PASSED';
  ...
  IF requires_provider AND array_length(_provider_ids, 1) > 0 THEN
    FOREACH current_provider_id IN ARRAY _provider_ids LOOP
      ...
      IF NOT EXISTS(SELECT 1 FROM provider_profiles WHERE id = current_provider_id) THEN
        RAISE EXCEPTION 'PROVIDER_NOT_FOUND: Provider profile % does not exist', current_provider_id;
      END IF;
      ...
      LOOP
        ...
        IF debug_count = 0 THEN
          ...
          RAISE EXCEPTION 'PROVIDER_UNAVAILABLE: Provider % not available for requested time slot %', current_provider_id, check_time;
        END IF;
      END LOOP;
      ...
      IF debug_count > 0 THEN
        RAISE EXCEPTION 'PROVIDER_CONFLICT: Provider % has conflicting appointment', current_provider_id;
      END IF;
  ...
  IF requires_shower THEN
    ...
    LOOP
      ...
      IF NOT FOUND THEN
        ...
        RAISE EXCEPTION 'SHOWER_SLOT_NOT_FOUND: No shower availability found for time slot %', check_time;
      END IF;
      ...
      IF shower_slot.available_spots <= 0 THEN
        ...
        RAISE EXCEPTION 'SHOWER_CAPACITY_EXCEEDED: Shower capacity exceeded for time slot %', check_time;
      END IF;
    END LOOP;
  ...
  -- 3. Create appointment with detailed error handling
  BEGIN
    ...
    RAISE NOTICE '[CREATE_BOOKING_ATOMIC] Appointment created successfully: %', appointment_id;
  EXCEPTION
    WHEN OTHERS THEN
      RAISE EXCEPTION 'APPOINTMENT_INSERT_FAILED: Failed to insert appointment - % %', SQLERRM, SQLSTATE;
  END;
  ...
  IF requires_provider AND array_length(_provider_ids, 1) > 0 THEN
    FOREACH current_provider_id IN ARRAY _provider_ids LOOP
      ...
      EXCEPTION
        WHEN OTHERS THEN
          RAISE EXCEPTION 'PROVIDER_LINK_FAILED: Failed to link provider % - % %', current_provider_id, SQLERRM, SQLSTATE;
      END;
    END LOOP;
  END IF;
  ...
  IF requires_shower THEN
    ...
    LOOP
      ...
      EXCEPTION
        WHEN OTHERS THEN
          RAISE EXCEPTION 'SHOWER_UPDATE_FAILED: Failed to update shower availability for slot % - % %', check_time, SQLERRM, SQLSTATE;
      END;
    END LOOP;
  END IF;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      RAISE NOTICE '[CREATE_BOOKING_ATOMIC] WARNING: Failed to create appointment event - % %', SQLERRM, SQLSTATE;
  END;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      RAISE NOTICE '[CREATE_BOOKING_ATOMIC] WARNING: Failed to queue notification - % %', SQLERRM, SQLSTATE;
  END;
